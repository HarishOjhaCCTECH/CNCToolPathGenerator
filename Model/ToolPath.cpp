	...
	for (int i = 9; i >= 0; i--)
	{
		for (int j = 9; j >= 0; j--)
		{
			for (int k = 9; k >= 0; k--)
			{
				...
				// when reaches the end of loop, i.e., vo line khatam ho gya
				if (k == 0)
				{
					if (!boxes[i][j][k].existence)
					{
...
const QVector<GLfloat>& ToolPath::ToolPathVertices() { return mToolPathVertices; }

int ToolPath::BoxCheck(const vector<vector<vector<BoundingBox>>>& boxes, int i, int j, int k)
{
	if (boxes[i][j][k].existence)
	{
